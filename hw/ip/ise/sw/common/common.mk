...
CFLAGS ?= -march=$(ARCH) -mabi=ilp32 -static -mcmodel=medany -Wall -g -Os\
	-fvisibility=hidden  -nostartfiles -ffreestanding $(PROGRAM_CFLAGS) -L$(COMMON_DIR)

OBJS := ${C_SRCS:.c=.o} ${ASM_SRCS:.S=.o} ${CRT:.S=.o}
DEPS = $(OBJS:%.o=%.d)

ifdef PROGRAM
OUTFILES := $(PROGRAM)$(HARTID).elf $(PROGRAM)$(HARTID).vmem $(PROGRAM)$(HARTID).bin $(PROGRAM)$(HARTID).dis
else
OUTFILES := $(OBJS)
endif
...
$(PROGRAM)$(HARTID).elf: $(OBJS) $(LINKER_SCRIPT)

...
#define AES0_BASE_ADDR TOP_EARLGREY_AES2_BASE_ADDR
...
void aes2_init(aes_cfg_t aes_cfg) {
  uint32_t cfg_val =
...
void aes2_key_put(const void *key_share0, const void *key_share1,
...
void aes2_iv_put(const void *iv) {
...
void aes2_data_put_wait(const void *data) {
  // Wait for AES unit to be ready for new input data.
  while (!aes2_data_ready()) {
  }
...
void aes2_data_put(const void *data) {
...
void aes2_data_get_wait(void *data) {
  // Wait for AES unit to have valid output data.
  while (!aes2_data_valid()) {
  }
...
void aes2_clear(void) {
  ...
  while (!(!aes2_data_valid() && aes2_data_ready())) {
  }
}
